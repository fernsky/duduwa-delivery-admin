...
export default function IncomeSourceSEO({
  ...
  const generateStructuredData = () => {
    ...
    const incomeSourceStats = overallSummary.map((item) => ({
      ...
      description: `${item.households.toLocaleString()} households in duduwa Rural Municipality depend on ${INCOME_SOURCE_NAMES_EN[item.incomeSource] || item.incomeSource} as their primary income source (${((item.households / totalHouseholds) * 100).toFixed(2)}% of total households)`,
    }));
    ...
    return {
      ...
      description: `Household income source distribution data across ${wardNumbers.length} wards of duduwa Rural Municipality with a total of ${totalHouseholds.toLocaleString()} households.`,
      keywords: [
      ...
      url: "https://duduwa-rm.gov.np/profile/economics/ward-wise-household-income-source",
      creator: {
        "@type": "Organization",
