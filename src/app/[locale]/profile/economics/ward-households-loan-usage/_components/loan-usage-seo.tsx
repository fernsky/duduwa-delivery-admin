...
export default function LoanUsageSEO({
  ...
  const generateStructuredData = () => {
    ...
    const loanUseStats = overallSummary.map((item) => ({
      "@type": "Observation",
      name: `${LOAN_USE_NAMES_EN[item.loanUse] || item.loanUse} loans in duduwa Rural Municipality`,
      ...
      description: `${item.households.toLocaleString()} households in duduwa Rural Municipality have taken loans for ${LOAN_USE_NAMES_EN[item.loanUse] || item.loanUse} purposes (${((item.households / totalHouseholds) * 100).toFixed(2)}% of total households with loans)`,
    }));

    return {
      "@context": "https://schema.org",
      "@type": "Dataset",
      name: "Loan Usage Patterns in duduwa Rural Municipality (डुडुवा गाउँपालिका)",
      description: `Loan usage purpose distribution data across ${wardNumbers.length} wards of duduwa Rural Municipality with a total of ${totalHouseholds.toLocaleString()} households with loans.`,
      keywords: [
      ...
      url: "https://duduwa-rm.gov.np/profile/economics/ward-households-loan-usage",
      creator: {
        "@type": "Organization",
